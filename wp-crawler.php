...
function wp_crawler_get_image($post_id, $image_url, $item_url) {
    ...
    $upload_folder = date('Y/m');
    $upload_dir   = wp_upload_dir();
    #var_dump($image_url);
    #var_dump($upload_dir);die;
    $filename = $upload_dir['basedir'] . '/' . $upload_folder . '/' . $post_id . '-' . $base_file_name;
    if ( false === file_exists( $upload_folder ) ) {
        chmod( $upload_dir['basedir'] . '/', 0777 );
        mkdir( $upload_dir['basedir'] . '/'. $upload_folder );
        chmod( $upload_dir['basedir'] . '/'. $upload_folder, 0777 );
    }
